...
class ColorFactory {
    ...
    constructor(cSpace) {
        ...
        this.newtonValues = ["1 0.2 0", "1 0.4 0", "1 0.6 0", "1 0.8 0", "1 1 0", "0.8 1 0", "0.6 1 0", "0.4 1 0", "0.2 1 0", "0 1 0", "0 1 0.2", "0 1 0.4", "0 1 0.6", "0 1 0.8", "0.00784313725490196 1 1", "0.011764705882352941 0.8 1", "0.0196078431372549 0.6 1", "0.0196078431372549 0.4 1", "0.023529411764705882 0.2 1", "0.023529411764705882 0 1", "0.2 0 1", "0.4 0 1", "0.6 0 1", "0.8 0 1", "1 0 1", "1 0 0.8", "1 0.00784313725490196 0.6", "1 0.011764705882352941 0.4", "1 0.01568627450980392 0.2"];
        ...
        /**
         * This sequence pairs the top and bottom colors in a sequence. 
         * It means that it takes:
         * the first red 5RP, then the last violet 10P, 
         * the second red 10RP, then the penultimate violet 5P,
         * and so on.
         */
        this.munsellValuesAlternated = [
            "0.8156862745098039 0 0.5058823529411764",
            "0.7254901960784313 0 0.6509803921568628",
    ...
    initialize(cSpace) {
        switch (cSpace) {
            ...
            case 'munsell':
                 ...
                 * and so on.
                 */
            case 'munsellAlternated':
                let munsellNames3 = ['5RP', '10P', '10RP', '5P', '5R', '10PB', '10R', '5PB', '5YR', '10B', '10YR', '5B', '5Y', '10BG', '10Y', '5GB', '5GY', '10G', '10GY', '5G']

                for (var i = this.munsellValuesAlternated.length - 1; i >= 0; i--) {
                    var tmp = this.munsellValuesAlternated[i].split(' ');
                    this.colors.push({ name: munsellNames3[i], chroma: chroma(tmp[0] * 255, tmp[1] * 255, tmp[2] * 255) });
                }
                break;
                // ---- MUNSELL INVERTED
