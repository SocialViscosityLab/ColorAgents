...
export var qualityChart = function (p5) {
  // The chart object
  let series;

  p5.setup = function () {
    p5.createCanvas(500, 350);
    series = new Chart(this, p5.createVector(30, 320), 430, 300, 1, "Model quality", -0.1, 0.1);
    // GUI Elements
  ...
  p5.draw = function () {
    ...
    world.permuLoaded.then((a) => {
      ...
      if (DOM.checkboxes.showGeneralModel.checked) {
        //go over all the keys of metrics.viscosityData
        series.geomPath(
          metrics.globalQualityData.map((q) => q.qValue),
          "global",
          [0, 0, 0],
        );
        series.geomPath(
          metrics.globalQualityData.map((q) => q.rqValue),
          "real global",
          [200, 200, 200],
        );
      }
